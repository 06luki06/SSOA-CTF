...
async def get_comments(
    recipient_id: int,  # Changed from UUID to int
    db: Session = Depends(get_db)
):
    comments = db.query(Comment).filter(Comment.recipient_id == recipient_id).all()
    result = [
        {
            "author_id": c.author_id,
            "author_name": db.query(User).filter(User.id == c.author_id).first().name,
            "comment": c.comment
        } for c in comments
    ]
